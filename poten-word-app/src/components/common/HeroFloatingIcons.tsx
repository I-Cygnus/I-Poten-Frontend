...
const Layer = styled.div<{ $w: string; $h: string; $top: string; $right: number; $debug?: boolean }>`
    ...
    > .field {
        ...
        right: calc(
            max(var(--poten-main-right-gap, 20px), env(safe-area-inset-right))
            + ${({ $right }) => $right}px
        );
        ...
        ${({ $debug }) =>
            $debug &&
            "outline: 2px dashed rgba(0,128,255,.6); background: rgba(0,128,255,.06);"}
    }

    @media (max-width: 768px) {
        > .field {
          right: calc(
            max(var(--poten-main-right-gap, 12px), env(safe-area-inset-right))
            + 0px
          );
        }
      }
