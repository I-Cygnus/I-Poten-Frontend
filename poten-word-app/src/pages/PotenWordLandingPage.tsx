...
async function attachJobGroupToFolder(wordbookId: string, jobKey: string) {
    const res = await http.post(
        `/me/folders/${wordbookId}/terms:bulk-by-job`,
        { jobKey }
    );
    return res.data;
}
...
export default function PotenWordLandingPage() {
    ...
    const handleSaveJobToNotebook = useCallback(
        async (wordbookId: string) => {
            ...
            try {
                setSaving(true);
                await attachJobGroupToFolder(wordbookId, selectedJob.key);
