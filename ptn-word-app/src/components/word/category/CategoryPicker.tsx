...
import { useCategoryTree, Category } from "../../../hooks/useCategoryTree.ts";
...
const CategoryDropdownPicker: React.FC<Props> = ({ initialPathIds }) => {
    const navigate = useNavigate();
    const location = useLocation();

    // URL(catPath) → 초기 선택 복원
    React.useLayoutEffect(() => {
        if (initialPathIds?.length) return; // prop 우선
        const sp = new URLSearchParams(location.search);
        const path = (sp.get("catPath") ?? "")
            .split("/")
            .map((s) => Number(s))
            .filter((n) => Number.isFinite(n) && n > 0);
        if (path.length) {
            setSel0((path[0] as number) ?? "");
            setSel1((path[1] as number) ?? "");
            setSel2((path[2] as number) ?? "");
        }
    ...
    // 현재 스키마(depth & parent_id)에 딱 맞게 lazy 로딩
    ...
    /** 상태별 플레이스홀더(요구한 UX 문구) */
    const ph0 = "대분류를 선택하세요.";
    const ph1 = sel0Num
        ? (level1.loading ? "불러오는 중..." : (level1.items.length ? "중분류를 선택하세요." : "하위 분류 없음"))
        : "중분류를 선택하세요.";
    const ph2 = sel1Num
        ? (level2.loading ? "불러오는 중..." : (level2.items.length ? "소분류를 선택하세요." : "하위 분류 없음"))
        : "소분류를 선택하세요.";
